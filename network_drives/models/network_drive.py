    ...
    def action_refresh_contents(self):
        ...
        if self.file_path:
            if self.is_networkdrive:
                driver_credential = self.driver_credential_id
                drive_letter = driver_credential.sudo().drive_letter
                network_share = fr'{self.file_path}'
                username = driver_credential.sudo().user_name  # or just "username" if not domain-based
                password = driver_credential.sudo().password
                ...
                # Use the drive
                try:
                    files = os.listdir(rf'{drive_letter}\\')
                    for file in files:
                        print(file)
                except Exception as e:
                    _logger.warning(f"Failed to access mapped drive: {network_share}")
